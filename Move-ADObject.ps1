...
Write-Host "`nPlease select an OU." -f Yellow

$ouSelection = Choose-ADOrganizationalUnit -HideNewOUFeature -Domain northside.local

if (!$ouSelection) {
    Write-Host "OU must be selected. Script ended." -f Red
    exit
}
...
ForEach ($c in $computers) {
    ...
    try {
        (Get-ADComputer $c).DistinguishedName | Move-ADObject -TargetPath $ouSelection.DistinguishedName -Credential $creds
        } catch {
            Write-Host "Could not move $c to $orgFormatted" -f Red
    }
    ...
    Write-Host "$c moved to " -NoNewline
    Write-Host "$orgFormatted" -f Cyan
}